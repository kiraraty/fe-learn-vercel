...
根据**页面开始加载**的时间报告可视区域内可见的**最大图像或文本块**完成渲染的计算时间，用于**测验加载性能**，**衡量网站初次载入速度**。 我们应该控制该值在**2.5 秒**以内
...
由于浏览器解析HTML是按照顺序解析的，当解析到某个元素的时候，觉得首屏完成了，就在此元素后面加入`script`标签计算首屏完成时间
...
Websocket是html5提出的一个协议规范，是为解决客户端与服务端实时通信。与传统的 http 协议不同，`Websocket` 是一个持久化的协议，该协议允许由服务器主动的向客户端推送信息。本质上是一个基于tcp，先通过HTTP/HTTPS协议发起一条特殊的http请求进行握手后创建一个用于交换数据的TCP连接。
...
![img](https://img-blog.csdn.net/20170922165100138?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvbGp4YmJzcw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/Center)三次握手，每次都需要授权等信息，一次request，一次response以后关闭连接
...
**![img](https://img-blog.csdn.net/20170922165320172?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvbGp4YmJzcw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/Center)三次握手，当经过第一次授权后，发起一次request，回复一次response，保持连接keep-alive的时间，这期间内可以发起多次通信，时间到达后关闭连接，需要注意的是每次发起一次通信后，keep-alive的时间会重新计时；**  
...
![在这里插入图片描述](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/imgsimgsmvHaji6xfMdy2h1.png)
