...
javascript语法：object.style.left="100px"
...
1.margin-left、margin-top会把**元素向左或向上拉，盖住其旁边的元素**。

2.margin-right、margin-bottom会把**相邻元素向左或向上拉，盖住设置了负外边距的元素**。
...
对**中间的div设置向上的负外边距**，可以看到**绿色块向上流动**，覆盖了部分红色块，蓝色块也跟着绿色块向上流动。 
...
1.margin-left、margin-top会使元素向左或向上移动，盖住其旁边的元素。
...
![img](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/20200330110901227.png)
 ...
 2.margin-right、margin-bottom会**把相邻元素向左或向上拉，盖住设置了负外边距的元素**。

#### **相对定位下设置负外边距：**

1.margin-left、margin-top会使元素向左或向上移动。

2.margin-right、margin-bottom会把相邻元素向左或向上拉，盖住设置了负外边距的元素。该元素本身不会移动，其他元素会向该元素移动一定的距离，但该元素会覆盖在其他元素之上。
...
设计一个没有设置宽度的块元素，或者（width: auto），该块元素被包含在宽度为400px,高度为50px的父元素内（居中）。

![img](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/Imgs/20200330161948234.png)

设置向右的负外边距margin-right: -100px，元素被向右拉长。
...
将元素的长宽都设置为0，显示出来的效果是这样的： ![img](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/imgsxODpCdEjHlLFcIt.webp)
...
打开 Chrome 浏览器，启动移动端调试模式，在控制台去输出这个 `devicePixelRatio` 的值。这里选中 iPhone6/7/8 这系列的机型，输出的结果就是2： ![a8af7dca29f84b7e9d1f94232713ef07~tplv-k3u1fbpfc...](https://femarkdownpicture.oss-cn-qingdao.aliyuncs.com/imgsK3JE6ISP5Wyf1ms.webp)这就意味着设置的 1px CSS 像素，在这个设备上实际会用 2 个物理像素单元来进行渲染，所以实际看到的一定会比 1px 粗一些。 **解决1px 问题的三种思路：**
...
实现一个自适应正方形关键就是使这个div的height=width

**利用CSS3新增的 vw, vh, vmin, vmax**
vw - 1/100视口宽度为一个单元；
vh - 1/100视口高度为一个单元;
...
**利用padding-top/bottom设为百分数撑开div**
CSS中padding-top/right/bottom/left, margin-top/right/bottom/left属性设为百分数时都是**相对于父元素的宽设定**。
...
1. 给.box增加内容会使高度溢出，也就是高度会拉长（因为内容会撑起盒模型的content部分而padding-bottom在其下方，background-color会填充content和padding的部分。）
...
2. 使用padding-top内容会显示在正方形下的白色区域。
